...
| 19 | M2 | An additive `:root` block supplies chip text tokens; §9.1's own tokens are untouched. | §9.2's semantic colour pairs fail WCAG 2.2 AA at the chip's 10px/600 weight — Stale measured 2.21:1 against a 4.5:1 requirement. The additive tokens bring all four chips to 4.6–8.9:1 in both themes without altering the design system §9.1 defines. |
...
| 21 | M2 | The badge colour is a hex literal in `badge.js`, outside `panel.css`'s `:root` (§17.7). | `chrome.action.setBadgeBackgroundColor` is drawn by Chrome and takes a literal — no stylesheet is reachable from a service worker. The value is §9.1's light-theme `--accent` copied verbatim. |
| 22 | M2 | Six files added beyond §2.1's tree: `background/{badge,changesApi}.js`, `panel/{dom,sources}.js`, `test/{changes,guards,panel.browser}.test.js`. | §17.10 caps files at ~500 lines and says to split when bigger; a single `background.js` or `panel.js` would have run past it. `guards.test.js` in particular turns §17.4's "grep-test yourself" instruction into an automated audit that fails CI if `state: "verified"` is ever assigned outside probe.js. |
