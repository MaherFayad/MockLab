...
| 27 | M3 | `extension/test/panel.browser.test.js` is 840 lines. | One Chromium launch and one demo server serve both the M2 changes flow and the M3 pick flow, and the pick test depends on the clean post-reset state the M2 flow leaves behind. Splitting it today buys a second ~30 s browser launch and a duplicated ~90-line Playwright resolver — §17.10's own drift problem in another shape. It gains State D and the probe progress card at M4 and will pass 1000 lines; the split then is by flow, with the harness moved to a 0-test `test/harness.js`. |
...
| 34 | M3 | `extension/src/background/pickMessages.js` declares the pick message types instead of `messages.js`. **Temporary.** | §17.8 requires one home for message constants, and `messages.js` was owned by another agent mid-milestone. Declared once, not duplicated, with a merge recipe in its header; both consumers import from it. Folded into `messages.js` as a no-behaviour-change commit — see the M3 section of `BUILD_LOG.md`. |
