...
| 30 | M3 | §6.1's smart-target walk accepts a parent on `area ratio ≤ 1.4` **OR** a border box within 24px on every side (text must still match; cap still 4 levels). | Measured in real Chromium: an area ratio depends on text *length* while padding does not, so the ratio is harshest on exactly the short text §6.1 names as its purpose. The demo's own pill (`padding: 5px 14px` at 12px) measures **2.71×**; a Bootstrap badge 1.86×, a Tailwind `px-2 py-0.5` 1.61×, a Material chip 2.14× — none pass 1.4×, and the demo's pill besides has no child element, so neither figure is even reachable on the DoD path. (**2.57× was recorded here first and is corrected** — it is a 5px/12px fixture the tests mislabel as the demo's own CSS.) No realistically padded pill passes 1.4× — the literal rule cannot do the job §6.1 describes for it. The inset half never rejects anything the ratio accepts, so nothing that worked before regresses; a same-text whole row (40×) is still blocked by both. |
