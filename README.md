...
|---|-----------|-----------|-----|
| 1 | M0 | The demo page fetches `api/trip.json` with `fetch()` but `api/user.json` with `XMLHttpRequest`. PLAN.md §14 describes both as fetches. | The demo is the acceptance harness for every milestone. Using both APIs means the harness exercises both interception paths in §5.1 (the `fetch` wrapper and the XHR `responseText` override) rather than only one. Still two sources, as §14 requires. |
| 2 | M0 | The demo page loads no web font; it uses Inter when the machine has it and falls back to the system UI face. PLAN.md §14 specifies Inter. | A probe run reloads the page up to eight times. A web-font request on every load would appear in the capture list and delay the network-quiet condition in §7.3's settle definition. The panel itself still imports Inter exactly as §9.1 requires. |
| 3 | M0 | Both workspaces run tests as `node --test test/*.test.js` rather than `node --test` over the directory. | Node 22 resolves a bare directory argument as a module path and fails. The glob form is what actually runs the suites. |
