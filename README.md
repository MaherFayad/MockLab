...
| 30 | M3 | §6.1's smart-target walk accepts a parent on `area ratio ≤ 1.4` **OR** a border box within 24px on every side (text must still match; cap still 4 levels). | Measured in real Chromium: an area ratio depends on text *length* while padding does not, so the ratio is harshest on exactly the short text §6.1 names as its purpose. The demo's own pill sits at **2.57×** and no realistically padded pill passes 1.4× — the literal rule cannot do the job §6.1 describes for it. The inset half never rejects anything the ratio accepts, so nothing that worked before regresses; a same-text whole row (40×) is still blocked by both. |
| 31 | M3 | §6.3's sibling-key gate fires on *any* hit from a needle derived from the element, not only a full-text hit. | Read strictly, the rule cannot fire in the example the same sentence gives: a pill reading "On time" has no verbatim match in `{"status":"ON_TIME"}`, so there is no full-text hit to hang off and `ON_TIME` is never found — making the M3 DoD, which names that exact case as a MUST, impossible. §17.11: prefer the working behaviour. A response with zero hits contributes no siblings, proved both ways, so nothing leaks in. |
