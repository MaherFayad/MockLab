...
| 11 | M1 | **`interceptor.js` is ~535 lines of code, above §17.10's ~500.** | §17.10 says "split when bigger", which this one file cannot do: a MAIN-world script has no module graph, so it cannot import anything (§17.2). Splitting it would mean a bundler, which §17.10 also forbids. Everything splittable already lives in `signatures.js`, `jsonpath.js` and `ruleStore.js`. |
